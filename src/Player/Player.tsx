...
function PlayerStats({ player, power, turn, points }: PlayerStatsProps) {
    ...
    function startCombat() {
        ...
        if (playersContext.gameplay['player_' + player + '_attack_power'] > playersContext.gameplay['player_' + opositePlayer + '_attack_power']) {
            ...
            updatePlayerStats(opositePlayer);
        ...
        if (playersContext[player].power === 0 || playersContext[opositePlayer].power === 0) {
            ...
            playersContext.gameplay.winner = getGameWinner();
        }

        // Reset player values after round is finish
        resetPlayersStatsAfterCombat();
    }

    function updatePlayerStats(plyr: string) {
        // Update points by round winner
        playersContext[plyr].points += POINTS_PER_WIN;
        // Update win count
        playersContext[plyr].winsCount++;
    }

    function getGameWinner() {
        // Check is left player winner
        if (playersContext.left.points > playersContext.right.points) {
            return 'left - POINTS: ' + playersContext.left.points + ' (power added - ' + playersContext.left.power + ')' + ' (rounds won - ' + playersContext.left.winsCount + ') ||| RIGHT statistics: POINTS: ' + playersContext.right.points + ' (power added - ' + playersContext.right.power + ')' + ' (rounds won - ' + playersContext.right.winsCount + ')';
            // Check is right player winner
        } else if (playersContext.left.points < playersContext.right.points) {
            return 'right - POINTS: ' + playersContext.right.points + ' (power added - ' + playersContext.right.power + ')' + ' (rounds won - ' + playersContext.right.winsCount + ') ||| LEFT statistics: POINTS: ' + playersContext.left.points + ' (power added - ' + playersContext.left.power + ')' + ' (rounds won - ' + playersContext.left.winsCount + ')';
            //It is draw
        }
    ...
    function resetPlayersStatsAfterCombat() {
